...
namespace ProtoVHDL
{
    ...
    public class CodeGen : ProtoCore.CodeGen
    {
        ...
        private void VHDL_EmitParallelComponentMultiplexer(
            string lhs, 
            ...
            int selecIndexSignalSize )
        {
            ...
            string multiplexerName = ProtoCore.VHDL.Utils.GenerateNameMultiplexerInputToParallelComponent(funcName, iterations);
            ...
            int maxElements)
        {
            ...
            string multiplexerName = ProtoCore.VHDL.Utils.GenerateNameMultiplexerInputToParallelComponent(functionCallName, iterations);
            ...
            for (int i = 0; i < parallelComponentInstance; ++i)
            {
                ...
                //
                //      op11	=> array_a(0),  -- Argument 1 list
                //      op21	=> array_a(3),
                //      op31	=> array_a(6),
                //
                //      op12	=> array_b(0),  -- Argument 2 list
                //      op22	=> array_b(3),
                //      op32	=> array_b(6),
                //
                //      op1 	=> ALU1_op1,
           ...
           int selecIndexSignalSize)
       {
            ...
            )
        {
            ...
            if (!allocateForBaseVar && !isProperty && core.ClassTable.IndexOf(ident) != ProtoCore.DSASM.Constants.kInvalidIndex)
                buildStatus.LogSemanticError(String.Format(ProtoVHDL.StringConstants.classNameAsVariableError, ident), null, line, col, graphNode);
...
#region SSA_INDEX_STMT_RIGHT
        ...
        */
        ...
        public int AllocateMemberVariable(int classIndex, int classScope, string name, int rank = 0, ProtoCore.Compiler.AccessSpecifier access = ProtoCore.Compiler.AccessSpecifier.kPublic, bool isStatic = false)
        {
            ...
            if (null == symnode)
            {
                buildStatus.LogSemanticError(String.Format(ProtoVHDL.StringConstants.memberVariableAlreadyDefined, name, core.ClassTable.ClassNodes[classIndex].name));
        ...
        private void EmitLanguageBlockNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.AssociativeGraph.GraphNode graphNode, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone)
        {
            if (IsParsingGlobal() || IsParsingGlobalFunctionBody() || IsParsingMemberFunctionBody() )
            {
                ...
                if (ProtoCore.Language.kAssociative == langblock.codeblock.language && !isTopBlock)
                {
                    ...
                    buildStatus.LogSyntaxError(ProtoVHDL.StringConstants.invalidNestedAssociativeBlock, core.CurrentDSFileName, langblock.line, langblock.col);
                }
            ...
            GraphNode graphNode = null)
        {
            ...
            if (ProtoCore.Compiler.Associative.CompilePass.kClassName == compilePass)
            {
                ...
                {
                    buildStatus.LogSemanticError(ProtoVHDL.StringConstants.classCannotBeDefinedInsideLanguageBlock, core.CurrentDSFileName, classDecl.line, classDecl.col);
                }
        ...
        private void EmitBinaryExpressionNode(AssociativeNode node, ref ProtoCore.Type inferedType, bool isBooleanOp = false, ProtoCore.AssociativeGraph.GraphNode graphNode = null, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, bool isTempExpression = false)
        {
            ...
            */
            ...
            else //(ProtoCore.DSASM.Operator.assign != b.Optr)
            {
                ...
                {
                    buildStatus.LogSemanticError(ProtoVHDL.StringConstants.functionPointerNotAllowedAtBinaryExpression, core.CurrentDSFileName, bnode.LeftNode.line, bnode.LeftNode.col);
                }
            ...
            if (bnode.Optr != ProtoCore.DSASM.Operator.assign)
            {
                ...
                {
                    buildStatus.LogSemanticError(ProtoVHDL.StringConstants.functionPointerNotAllowedAtBinaryExpression, core.CurrentDSFileName, bnode.RightNode.line, bnode.RightNode.col);
                }
                ...
                else
                {
                    ...
                    }
                    {
                        ...
                        if (procNode != null)
                        {
                            ...
                            {
                                buildStatus.LogSemanticError(String.Format(ProtoVHDL.StringConstants.functionAsVariableError, t.Name), core.CurrentDSFileName, t.line, t.col);
                            }
                        }
                    }
